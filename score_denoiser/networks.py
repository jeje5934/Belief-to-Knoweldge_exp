# Minimal extraction from NVlabs/edm  (training/networks.py)
# Original: https://github.com/NVlabs/edm
# License: CC BY-NC-SA 4.0
#
# Modifications:
#   - Removed @persistence.persistent_class decorator (not needed for training)
#   - Removed torch_utils dependency
#   - Kept SongUNet + EDMPrecond only
...
class Conv2d(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel, bias=True,
                 up=False, down=False, resample_filter=[1, 1],
                 fused_resample=False, init_mode='kaiming_normal',
...
class AttentionOp(torch.autograd.Function):
    ...
    def backward(ctx, dw):
        ...
        db = torch._softmax_backward_data(
            grad_output=dw.to(torch.float32), output=w.to(torch.float32),
...
class UNetBlock(torch.nn.Module):
    def __init__(self, in_channels, out_channels, emb_channels,
                 ...
                 num_heads=None, channels_per_head=64, dropout=0,
                 skip_scale=1, eps=1e-5, resample_filter=[1, 1],
                 resample_proj=False, adaptive_scale=True,
                 init=dict(), init_zero=dict(init_weight=0), init_attn=None):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.emb_channels = emb_channels
        self.num_heads = (0 if not attention
                          else num_heads if num_heads is not None
        ...
        self.conv1 = Conv2d(in_channels=out_channels, out_channels=out_channels,
                            kernel=3, **init_zero)
...
# ---------------------------------------------------------------------------
# SongUNet: DDPM++ / NCSN++ backbone from EDM
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# EDM preconditioning: D(x;σ) = c_skip·x + c_out·F_θ(c_in·x; c_noise)
# ---------------------------------------------------------------------------
...
class EDMPrecond(torch.nn.Module):
    def __init__(self,
        ...
        _models = dict(SongUNet=SongUNet)
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        ...
        class_labels = (None if self.label_dim == 0
        ...
        F_x = self.model(
            (c_in * x).to(dtype), c_noise.flatten(),
